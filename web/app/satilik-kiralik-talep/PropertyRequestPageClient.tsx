...
type Settings = {
  ...
  propertyRequestPageTitle?: string;
  propertyRequestPageDescription?: string;
  [key: string]: unknown;
};
...
export default function PropertyRequestPageClient() {
  ...
  const [requestType, setRequestType] = useState<"SELL" | "RENT_OUT">("SELL");

  useEffect(() => {
    // URL parametresinden tip belirleme
    if (typeParam === "rent" || typeParam === "RENT_OUT") {
      setRequestType("RENT_OUT");
    } else {
      setRequestType("SELL");
    }
  }, [typeParam]);
  ...
  const pageTitle = requestType === "SELL" ? "Satmak İstiyorum" : "Kiralamak İstiyorum";
  const pageDescription = requestType === "SELL" 
    ? "Gayrimenkulünüzü en iyi fiyata satmak için profesyonel destek alın"
    : "Gayrimenkulünüzü güvenle kiraya vermek için bize ulaşın";
        ...
        <div className="container" style={{ maxWidth: 900, margin: "0 auto", padding: "0 20px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 16, marginBottom: 32 }}>
            <button
              ...
              style={{
                ...
                borderRadius: 8,
                border: "2px solid",
                borderColor: requestType === "SELL" ? "#0a4ea3" : "#e5e7eb",
                background: requestType === "SELL" ? "#0a4ea3" : "#fff",
                color: requestType === "SELL" ? "#fff" : "#374151",
                cursor: "pointer",
                transition: "all 0.2s",
              }}
            >
              <i className="fa-solid fa-tag" style={{ marginRight: 8 }}></i>
              Satmak İstiyorum
            </button>
            <button
              onClick={() => setRequestType("RENT_OUT")}
              style={{
                padding: "14px 32px",
                fontSize: 16,
                fontWeight: 600,
                borderRadius: 8,
                border: "2px solid",
                borderColor: requestType === "RENT_OUT" ? "#0a4ea3" : "#e5e7eb",
                background: requestType === "RENT_OUT" ? "#0a4ea3" : "#fff",
                color: requestType === "RENT_OUT" ? "#fff" : "#374151",
                cursor: "pointer",
                transition: "all 0.2s",
              }}
            >
              <i className="fa-solid fa-key" style={{ marginRight: 8 }}></i>
              Kiralamak İstiyorum
            </button>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 16, marginBottom: 32 }}>
            <div style={{ background: "#eff6ff", borderRadius: 12, padding: 20, textAlign: "center" }}>
              <i className="fa-solid fa-handshake" style={{ fontSize: 32, color: "#3b82f6", marginBottom: 12 }}></i>
              <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>Güvenli İşlem</h3>
              <p style={{ fontSize: 14, color: "#6b7280", margin: 0 }}>
                {requestType === "SELL" ? "En iyi alıcıyı buluyoruz" : "Güvenilir kiracı garantisi"}
              </p>
            </div>
            <div style={{ background: "#f0fdf4", borderRadius: 12, padding: 20, textAlign: "center" }}>
              <i className="fa-solid fa-chart-line" style={{ fontSize: 32, color: "#22c55e", marginBottom: 12 }}></i>
              <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>Piyasa Analizi</h3>
              <p style={{ fontSize: 14, color: "#6b7280", margin: 0 }}>
                {requestType === "SELL" ? "Doğru fiyatlandırma" : "Optimal kira bedeli"}
              </p>
            </div>
            <div style={{ background: "#fef3c7", borderRadius: 12, padding: 20, textAlign: "center" }}>
              <i className="fa-solid fa-bullhorn" style={{ fontSize: 32, color: "#f59e0b", marginBottom: 12 }}></i>
              <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>Geniş Pazarlama</h3>
              <p style={{ fontSize: 14, color: "#6b7280", margin: 0 }}>Tüm platformlarda ilan yayını</p>
            </div>
          </div>

          {/* FORM */}
          <PropertyRequestForm formType="property" requestType={requestType} />
        </div>
      </main>
