...
export default function AdminValuationRequestsPage() {
  const [requests, setRequests] = useState<ValuationRequest[]>([]);
  const [stats, setStats] = useState<Stats | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [statusFilter, setStatusFilter] = useState("");
  const [selectedIds, setSelectedIds] = useState<string[]>([]);
  const [detailModal, setDetailModal] = useState<ValuationRequest | null>(null);
  ...
  const loadData = async () => {
    ...
    try {
      const [requestsRes, statsRes] = await Promise.all([
        fetch(`${API_BASE_URL}/valuation-requests${statusFilter ? `?status=${statusFilter}` : ""}`, { headers }),
        fetch(`${API_BASE_URL}/valuation-requests/stats`, { headers }),
      ]);
  ...
  }, [statusFilter, isAuthed]);

  const updateStatus = async (id: string, newStatus: string) => {
    const token = getToken();
    try {
      const res = await fetch(`${API_BASE_URL}/valuation-requests/${id}/status`, {
        method: "PATCH",
        headers: {
  ...
  const handleExport = () => {
    ...
    window.open(`${API_BASE_URL}/valuation-requests/export?ids=${ids}&token=${token}`, "_blank");
  };
        ...
        <div className="admin-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <div>
            <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0 }}>
              <i className="fa-solid fa-calculator" style={{ marginRight: 12, color: "#0a4ea3" }}></i>
              Değerleme Talepleri
            </h1>
            <p style={{ color: "#6b7280", margin: "4px 0 0" }}>Evim Ne Kadar Eder formundan gelen talepler</p>
          </div>
        ...
        {stats && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }}>
            <div className="card" style={{ padding: 20, textAlign: "center" }}>
              ...
              <div style={{ color: "#6b7280", fontSize: 14 }}>Toplam Talep</div>
            </div>
            <div className="card" style={{ padding: 20, textAlign: "center", borderLeft: "4px solid #3b82f6" }}>
              <div style={{ fontSize: 32, fontWeight: 700, color: "#3b82f6" }}>{stats.new}</div>
              <div style={{ color: "#6b7280", fontSize: 14 }}>Yeni</div>
            </div>
            <div className="card" style={{ padding: 20, textAlign: "center", borderLeft: "4px solid #f59e0b" }}>
              <div style={{ fontSize: 32, fontWeight: 700, color: "#f59e0b" }}>{stats.inProgress}</div>
              <div style={{ color: "#6b7280", fontSize: 14 }}>İşlemde</div>
            </div>
            <div className="card" style={{ padding: 20, textAlign: "center", borderLeft: "4px solid #22c55e" }}>
              <div style={{ fontSize: 32, fontWeight: 700, color: "#22c55e" }}>{stats.closed}</div>
              <div style={{ color: "#6b7280", fontSize: 14 }}>Tamamlandı</div>
            </div>
          </div>
        )}
