...
const multerOptions = {
  storage: diskStorage({
    destination: './uploads/property-requests',
...
export class PropertyRequestsController {
  ...
  @Post()
  async create(@Body() dto: CreatePropertyRequestDto) {
  ...
  async uploadFiles(@UploadedFiles() files: Express.Multer.File[]) {
    const urls = files.map((file) => `/uploads/property-requests/${file.filename}`);
  ...
  async findAll(@Query('status') status?: string, @Query('type') type?: string) {
    ...
    return this.service.findAll(requestStatus, type);
  }
  ...
  async exportToExcel(@Query('ids') ids: string, @Res() res: Response) {
    ...
    const worksheet = workbook.addWorksheet('Satış/Kiralama Talepleri');

    // Başlıklar
    worksheet.columns = [
      { header: 'Tarih', key: 'createdAt', width: 20 },
      { header: 'Talep Tipi', key: 'requestType', width: 15 },
    ...
    data.forEach((item) => {
      worksheet.addRow({
        ...
        requestType: item.requestType === 'SELL' ? 'Satmak İstiyorum' : 'Kiralamak İstiyorum',
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=satis-kiralama-talepleri-${new Date().toISOString().slice(0, 10)}.xlsx`,
    );
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(Role.ADMIN, Role.MANAGER, Role.BROKER, Role.FIRM_OWNER, Role.REAL_ESTATE_EXPERT)
  async updateStatus(@Param('id') id: string, @Body() dto: UpdatePropertyRequestStatusDto) {
